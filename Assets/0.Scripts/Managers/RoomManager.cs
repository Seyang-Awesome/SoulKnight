...
{
    public InvokableAction<List<Vector2>, int> OnRequestSpawnEnemy = new();
    public InvokableAction OnEnemyClear = new();
}
    ...
    private void SetRoad(Room baseRoom, Room linkedRoom)
    {
        ...
        while (startPos - endPos != roadDirection)
        {
            RoadMatch roadMatch1 =
                roadConfig.roadMatches[Random.Range(0, roadConfig.roadMatches.Count)];
            ...
            {
                shadow.SetTile(wallPos1,roadConfig.shadow);
                shadow.SetTile(wallPos2,roadConfig.shadow);
            }

            for (int i = -Consts.RoadHalfWidth; i <= Consts.RoadHalfWidth; i++)
            {
                TileBase backgroundTile =
                    roadConfig.backgroundTiles[Random.Range(0, roadConfig.backgroundTiles.Count)];
